...
function loadPageData() {
    fetch('Department_of_Wildlife_Conservation.json')
        .then(response => response.json())
        .then(data => {
            Object.assign(dwcData, data);
            updatePageContent();
        })
...
function updatePageContent() {
    const storedData = localStorage.getItem('dwcData');
    if (storedData) {
        Object.assign(dwcData, JSON.parse(storedData));
    }
    document.querySelectorAll('[x-text]').forEach(element => {
        const key = element.getAttribute('x-text');
        element.innerText = dwcData[key];
    });
}

//funtion and style to the pop up
function openPopup() {
    ...
    popup.document.write(`
        ...
        <body>
            <div class="container">
                ...
                <textarea id="editableContent">${JSON.stringify(dwcData, null, 2)}</textarea>
            ...
            <script>
                function saveContent() {
                    ...
                    localStorage.setItem('dwcData', JSON.stringify(editedContent));
...
//function to update the content and once when refreshed it restores the orginal content
