...
function loadPageData() {
    fetch('wilpatu_National_park.json')
        .then(response => response.json())
        .then(data => {
            Object.assign(wnpData, data);
            updatePageContent();
        })
...
function updatePageContent() {
    const storedData = localStorage.getItem('wnpData');
    if (storedData) {
        Object.assign(wnpData, JSON.parse(storedData));
    }
    document.querySelectorAll('[x-text]').forEach(element => {
        const key = element.getAttribute('x-text');
        element.innerText = wnpData[key];
    });
}
    ...
    popup.document.write(`
        ...
        <body>
            <div class="container">
                ...
                <textarea id="editableContent">${JSON.stringify(wnpData, null, 2)}</textarea>
            ...
            <script>
                function saveContent() {
                    ...
                    localStorage.setItem('wnpData', JSON.stringify(editedContent));
